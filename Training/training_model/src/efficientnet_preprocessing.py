...
def create_augmentation_pipeline(seed=42, strength='high'):
    """
    Create an augmentation pipeline with configurable strength
    Enhanced for EfficientNet model training
    """
    # Base augmentations for all strength levels
    base_augmentations = [
        tf.keras.layers.RandomFlip("horizontal", seed=seed),
        tf.keras.layers.RandomFlip("vertical", seed=seed),  # Added vertical flip
        tf.keras.layers.RandomRotation(0.3, seed=seed),  # Increased rotation
    ]
    ...
    if strength == 'medium' or strength == 'high':
        base_augmentations.extend([
            ...
            tf.keras.layers.RandomBrightness(0.3, seed=seed),  # Increased brightness variation
            tf.keras.layers.RandomContrast(0.2, seed=seed),  # Added for all medium/high
        ])
...
# Optimized YOLODetectionGenerator class for EfficientNet
class EfficientNetGenerator(tf.keras.utils.Sequence):
    def __init__(
        ...
        augmentation_strength='high',  # Default to high for better performance
        image_size=(224, 224)  # Configurable image size
    ):
        super().__init__()
        self.df = dataframe
    ...
    def __getitem__(self, idx):
        ...
        for i, idx in enumerate(batch_indices):
            try:
                ...
                # Apply preprocessing for EfficientNet
...
# The rest of the functions remain similar but with optimizations
...
def create_efficientnet_generators(
    ...
    """Create data generators for training and validation using EfficientNet"""
    ...
    # Calculate class weights for training with improved balancing
    ...
    for i in range(len(diagnosis_to_idx)):
        ...
        if n_pos > 0:
            # Use square root to moderate extreme weights
            pos_weight = np.sqrt(n_neg / n_pos)
        else:
            pos_weight = 1.0
            
        # Cap weights to prevent extreme values but allow more range
        pos_weight = min(max(pos_weight, 0.1), 15.0)
    ...
    train_generator = EfficientNetGenerator(
        train_df,
        diagnosis_to_idx,
    ...
    val_generator = EfficientNetGenerator(
        val_df,
        diagnosis_to_idx,
        batch_size=batch_size,
        is_training=False,
...
def analyze_dataset(csv_path, image_dir, labels_dir, metadata_csv_path=None):
    ...
    for i in range(len(diagnosis_to_idx)):
        ...
        if n_pos > 0:
            pos_weight = np.sqrt(n_neg / n_pos)  # Use square root to moderate extreme weights
        else:
            pos_weight = 1.0
        class_weights[i] = pos_weight
...
def create_optimized_dataset(generator, is_training=True):
    ...
    # Define a generator function that yields batches
    def gen_fn():
        for i in range(len(generator)):
            x, y = generator[i]
            yield x, y
    
    # Create dataset with the correct output shapes and types
    output_signature = (
        tf.TensorSpec(shape=(generator.batch_size, *generator.image_shape), dtype=tf.float32),
        tf.TensorSpec(shape=(generator.batch_size, generator.n_classes), dtype=tf.float32)
    )
    ...
    # Apply optimizations
    if is_training:
        # For training, enable shuffling and prefetching
        dataset = dataset.shuffle(buffer_size=min(1000, len(generator) * generator.batch_size))
        dataset = dataset.prefetch(tf.data.AUTOTUNE)
        dataset = dataset.cache()
    else:
        # For validation, just prefetch
        dataset = dataset.prefetch(tf.data.AUTOTUNE)
